...
- `executable_group`: the *name* of the group that should receive the executable,
  ...
  of the entrypoint, or None to let the packager decide.
...
def _make_runfilesgroupinfo_init(*, entries, executable_group = None):
    ...
    if executable_group != None and type(executable_group) != "string":
        fail("RunfilesGroupInfo: executable_group must be a group name (string) or None, got ", type(executable_group))
...
RunfilesGroupInfo, _ = provider(
    ...
    fields = {
        ...
        "executable_group": "str or None: name of the group that carries the executable and its supporting files.",
    },
)
